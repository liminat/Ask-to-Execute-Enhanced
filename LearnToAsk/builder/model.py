...
class Builder(nn.Module):
    ...
    def forward(self, encoder_inputs, grid_repr_inputs, action_repr_inputs, label, location_mask=None, task_name='learn_to_ask'):
        dialogue_repr = self.encoder(encoder_inputs)
        if task_name == 'learn_to_ask':
            loss, action_type_logits = self.decoder(dialogue_repr, grid_repr_inputs, action_repr_inputs, label, location_mask, task_name)
            return loss, action_type_logits
        else:
            loss, acc, predicted_seq = self.decoder(dialogue_repr, grid_repr_inputs, action_repr_inputs, label, location_mask, task_name)
...
class UtteranceEncoder(nn.Module):
    ...
    def forward(self, encoder_inputs):
        ...
        # output_unpacked: [batch_size, seq_len, num_directions * hidden_size], [1, 25, 200]
        # hidden: [num_layers * num_directions, batch_size, hidden_size], [4, 1, 100]
...
class ActionDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.beta_0 = config['loss_weight'][0]
        self.beta_1 = config['loss_weight'][1]
        self.beta_2 = config['loss_weight'][2]

    def forward(self, utter_vec, world_repr, last_action, label, location_mask=None, task_name='learn_to_ask'):
        if task_name == 'learn_to_ask_or_execute':
            """
            ...
            if not self.training:
                ...
                for k in range(action_len):
                    ...
                    total_clarification += sum(action_type_label == 3)
                    total_clarification_correct += sum(action_type_pred[action_type_label == 3] == action_type_label[action_type_label == 3])

                    predicted_seq.append([location_pred, action_type_pred, color_pred])
                    if action_type_pred >= 2: break
                return (total_location_loss, total_action_type_loss, total_color_loss), (total_action_type_correct, total_location, total_location_correct, total_color, total_color_correct, total_clarification, total_clarification_correct), predicted_seq
            else:                
                ...
                return (total_location_loss, total_action_type_loss, total_color_loss), (total_action_type_correct, total_location, total_location_correct, total_color, total_color_correct, total_clarification, total_clarification_correct), None

        elif task_name == 'learn_to_ask':
            """
            utter_vec: [batch_size, seq_len, hidden_size]
            world_repr: [batch_size, 8, 11, 9, 11]
            last_action: [batch_size, 11]
            label: [batch_size]
            """
            _, action_type_logits, _ = self._get_logits(utter_vec, world_repr, last_action, location_mask)
            loss = self.CELoss(action_type_logits, label)
            return loss, action_type_logits
        else:
    ...
    def compute_loss(self, location_logits, action_type_logits, color_logits, location_label, action_type_label, color_label, loss_fn):
        location_loss = self.beta_0 * loss_fn(location_logits[action_type_label < 2], location_label[action_type_label < 2])
        action_type_loss = self.beta_1 * loss_fn(action_type_logits, action_type_label)
        color_loss = self.beta_2 * loss_fn(color_logits[action_type_label==0], color_label[action_type_label==0])
    ...
    def compute_valid_loss(self, location_logits, action_type_logits, color_logits, location_label, action_type_label, color_label, loss_fn):
        # TODO: will this loss push model to predict action_type_label???
        action_type_loss = 0
        location_loss = 0
        color_loss = 0
